...
block.textContent = "Этот блок был создан в JS  ";

block.style.backgroundColor = 'lightblue'; // Цвет фона
block.style.color = 'black';               // Цвет текста
block.style.fontSize = '20px';             // Размер текста
...
for (let i = 0; i < coursesAndDurationArray.length; i++) {
    ...
    courseBlock.innerHTML = `<strong>Title:</strong> ${course.title} <br> <strong>Duration:</strong> ${course.monthDuration} months`;
...
for (let i = 0; i < simpsons.length; i++) {
    ...
    ageParagraph.textContent = `Age: ${member.age}`;

    let infoParagraph = document.createElement('p');
    infoParagraph.textContent = member.info;

    let photoImg = document.createElement('img');
    photoImg.src = member.photo;
    photoImg.alt = `${member.name} ${member.surname}`;

    memberBlock.appendChild(nameHeading);
    memberBlock.appendChild(ageParagraph);
    memberBlock.appendChild(infoParagraph);
    memberBlock.appendChild(photoImg);
...
function createCourseBlock(course) {
    ...
    titleBlock.innerHTML = `<strong>Title:</strong> ${course.title}`;
    ...
    durationBlock.innerHTML = `<strong>Month Duration:</strong> ${course.monthDuration} <br> <strong>Hour Duration:</strong> ${course.hourDuration}`;
    ...
    modulesBlock.innerHTML = `<strong>Modules:</strong>`;
...
coursesArray.forEach(course => {
    let courseBlock = createCourseBlock(course);
    document.body.appendChild(courseBlock);
});